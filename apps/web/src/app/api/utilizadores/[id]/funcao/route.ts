...
// PATCH /api/utilizadores/:id/funcao (ADMINISTRADOR)
export async function PATCH(request: NextRequest, { params }: Params) {
  try {
    ...
    const { funcao } = await request.json();
    const utilizador = await utilizadorService.updateFuncao(id, { funcao }, auth.user.id);
