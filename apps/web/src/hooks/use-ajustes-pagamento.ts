...
import {
  ajustePagamentoApi,
  type CriarAjusteDTO,
  type RedefinirPrecoDTO,
} from "@/lib/api/ajustes-pagamento";
...
export function useRedefinirPreco() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: RedefinirPrecoDTO) => ajustePagamentoApi.redefinir(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ajustes-pagamento"] });
      queryClient.invalidateQueries({ queryKey: ["reservas"] });
      queryClient.invalidateQueries({ queryKey: ["entradas-livres"] });
